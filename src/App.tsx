...
const renderFormattedPrompt = (text: string) => {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('■')) {
      return (
        <span key={i} className="font-bold text-[14px] tracking-wide text-white/90">
          {part}
        </span>
      );
    }
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-8 gap-2 relative z-20">
              ...
              {sajuDb.job_dropdown.map((group) => {
                ...
                const displayTitle = isSelected ? `${group.name}\n(${selectedJobObj.name})` : group.name;
                ...
                return (
                  <div key={group.id} className="relative flex">
                    ...
                    >
                      <span className="text-[10px] font-bold leading-tight whitespace-pre-wrap break-keep">{displayTitle}</span>
                    </button>
